...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const failedRecipients = audit.recipients.filter((recipient) => recipient.normalizedStatus === 'failed' || recipient.normalizedStatus === 'failed_fetch' || recipient.normalizedStatus === 'textgrid_http_400');
